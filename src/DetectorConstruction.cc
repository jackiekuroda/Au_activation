...
void   DetectorConstruction::ConstructHPGeDetector(G4LogicalVolume* motherLogicalVolume)
{
	...
	G4double outerDeadLayerThick = GetouterDeadLayerThick();
	...
	logOuterDeadLayer ->SetUserLimits(stepLimit);
	logInnerDeadLayer ->SetUserLimits(stepLimit);
	logholder ->SetUserLimits(stepLimit);
	logmylar ->SetUserLimits(stepLimit);
	logkapton ->SetUserLimits(stepLimit);
	logShell ->SetUserLimits(stepLimit);
